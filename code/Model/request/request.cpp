...
double qa_loss_weight = 1.0, qa_mf_weight = 1.0;
real lr;
...
line_node node_M, node_F, node_Y, node_M_QA;
line_link link_MF, link_MY, link_FY, link_MF_QA, link_MM_QA;

double weight[5] = {1, 1, 1, 1, 1};
...
void *train_miniBatch_thread(void *id)
{
    ...
    while (1)
    {
        ...
        if (netid == 3) link_MF_QA.train_miniBatch_sg_edge_sampling(error_vec_u, error_vec_v, lr, next_random, func_rand_num);
        if (netid == 4) link_MM_QA.train_miniBatch_qa(error_vec_u, lr, alpha, func_rand_num);
...
void TrainModel() {
    ...
    weight[3] = qa_loss_weight;
    weight[4] = qa_mf_weight;
    ws = ransampl_alloc(5);
    ...
    sprintf(file_name, "%srm/mention_qa.txt", file_path);
    node_M_QA.init(file_name, vector_size);
    ...
    sprintf(file_name, "%srm/mention_feature_qa.txt", file_path);
    link_MF_QA.init(file_name, &node_M_QA, &node_F, negative);

    sprintf(file_name, "%srm/mention_pairs_qa.txt", file_path);
    link_MM_QA.init(file_name, &node_M_QA, &node_M_QA, negative);
    link_MM_QA.set_equal_edge_probability();
    ...
    printf("#RM: %d, #RM_QA: %d, #RM feature: %d, #RM type: %d\n", node_M.get_num_nodes(), node_M_QA.get_num_nodes(), node_F.get_num_nodes(), node_Y.get_num_nodes());
    printf("MF: %d, MY: %d, FY: %d\n", link_MF.get_num_edges(), link_MY.get_num_edges(), link_FY.get_num_edges());
    printf("MF_QA: %d, MM_QA: %d\n", link_MF_QA.get_num_edges(), link_MM_QA.get_num_edges());
    clock_t start = clock();
...
int main(int argc, char **argv) {
    ...
    if ((i = ArgPos((char *)"-qaMFWeight", argc, argv)) > 0) qa_mf_weight = atof(argv[i + 1]);
    if ((i = ArgPos((char *)"-qaMMWeight", argc, argv)) > 0) qa_loss_weight = atof(argv[i + 1]);
