...
def pipeline(json_file, brown_file, outdir, requireEmType, isEntityMention):
    ...
    print 'start training rms feature generation'
    mention_count = 0
...
def pipeline_qa(json_file, brown_file, featurefile, labelfile, outdir, requireEmType, isEntityMention):
    feature_map = load_map_qa(featurefile)
    label_map = load_map(labelfile)

    reader = MentionReader(json_file)
    ner_feature = NERFeature(is_train=True, brown_file=brown_file, requireEmType=requireEmType, isEntityMention=isEntityMention, feature_mapping=feature_map, label_mapping=label_map)
...
def load_map_qa(input):
    ...
    for line in f:
        seg = line.strip('\r\n').split('\t')
        mapping[seg[0]] = [int(seg[1]), int(seg[2])]
    f.close()
    return mapping
...
def filter(featurefile, trainfile, featureout,trainout, thres):
    f = open(featurefile)
    featuremap = {}
    old2new = {}
    count = 0
